...
\begin{tikzpicture}[scale=1.3]	
    ...
    \node[test] (test) at (0,-1)  {condition};
    \node[instr] (instr1) at (0,-2)  {bloc};
    \node[instr] (suite) at (0,-3)  {suite};

    \draw (instr1) edge (test)
          (instr1) edge[left, "break", dashed] (suite)
          (instr1) edge[left, bend left=90, looseness=2,
                        "continue", dashed] (test)
          (test) edge[vrai,bend right] (instr1)
