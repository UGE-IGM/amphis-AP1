...
\hypersetup{
  ...
  allcolors=.,
  urlcolor=DarkBlue,
  pdftitle=Algorithmique et Programmation 1 - CM4
}
...
\title{\includegraphics[width=0.75\textwidth]{../../img/logo-igm.png} \\[1em] Algorithmique et Programmation 1}
\subtitle{Approfondissements sur les fonctions}
...
\begin{frame}{Wooclap}
  \includegraphics[width=\textwidth]{img/Wooclap.png}

  \url{https://app.wooclap.com/PQZJFT}
\end{frame}


\begin{frame}{CC0 : un petit bilan}
  \includegraphics[width=\textwidth]{img/Bilan_CC0.png}

  Moyenne : 14,67/20 \qquad Médiane : 16/20
  % Questions les moins réussies : algo max 1, exec et while tableau 2
\end{frame}
...
\begin{frame}[fragile]{Fonction sans valeur de retour}
  ...
  Dessiner un carré fait du caractère \mintinline{python}{carac} (dans Thonny) :
\end{column}
...
def dessine_carre(n, carac):
    ...
    while i < n:
        ...
        print(ligne)
        i += 1
    return  # Facultatif : ne renvoie rien
\end{minted}
  \onslide<3>
  Mieux :
\begin{minted}{python}
...
\begin{frame}[fragile]{Composition de fonctions : minimum d'une liste}
  Calculer le \emph{minimum} d'une liste non-vide (dans Thonny) :
  \pause
\begin{overprint}
  \onslide<1>
...
\end{minted}
  \end{block}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \texttt{minimum\_liste\_input\_print.py}
    \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
  ...
  Les espaces de noms sont imbriqués :
  \begin{itemize}
  \item espace de noms prédéfini (\mintinline{python}{print}, \mintinline{python}{input}, \dots)
  \item espace de noms global : définis dans le programme principal (\emph{variables globales})
...
\begin{frame}{Portée des variables}
  ...
  \item la plupart du temps, on n'a besoin que des variables locales

    (quitte à ajouter des paramètres)
  \end{itemize}
\end{block}
...
\begin{block}{Affectation}
  \vspace{0pt}
  Seulement les variables \alert{locales} de la fonction courante !

  (ou globales \emph{si on est dans le programme principal})
\end{block}
...
\begin{minted}{python}
def minimum_int(a: int, b: int) -> int:
\end{minted}
\end{exampleblock}
\end{frame}
...
\begin{frame}{Responsabilité}
  ...
  En général, une fonction est \emph{responsable} d'\alert{une seule chose}.

  Sinon, on fait \emph{plusieurs} fonctions.

      Ce que ça donne dans Thonny :

      \texttt{minimum\_liste\_responsabilite.py}
\end{frame}

\begin{frame}[fragile]{Un autre exemple de composition de fonctions : dessiner un carré}
  \begin{columns}
    \begin{column}{0.1\textwidth}
\begin{verbatim}
*****
...
On peut donc décomposer ainsi :
  ...
  \item
\begin{minted}{python}
def dessine_ligne(n,carac):
    """Dessine *une ligne* de longueur n
       composée du caractère carac."""
\end{minted}
  \item
\begin{minted}{python}
def dessine_carre(n,carac):
    """Dessine *un carré* de longueur n
       composé du caractère carac."""
\end{minted}
  \end{itemize}
