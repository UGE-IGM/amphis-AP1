...
\newcommand{\wandicon}{\reflectbox{\includegraphics[width=2.5 cm]{Images/baguette_magique.jpg}}}
...
\begin{frame}{Quelques questions pour commencer !}
  ...
  cf \href{https://app.wooclap.com/events/PLJFIOX/live-session}{Wooclap}.
...
Chaque semaine :
    \begin{itemize}
        ...
        \item 2 séances de TP
            \begin{itemize}
                \item 1 TP \og découverte \fg{}/entraînement
                \item 1 TP \og exo+\fg{} : mini-projet, créativité, autonomie,...
            \end{itemize}
    \end{itemize}
    \end{block}
    \pause
  ...
  \begin{frame}{Planning}
  	...
  	\includegraphics[width=\textwidth,
  			...
  			keepaspectratio]{Images/Planning.png}
  	\end{center}

  \end{frame}

\begin{frame}{Assiduité en cours}
	\begin{center}
		La présence en CM, en TD, et en TP est \textbf{obligatoire}.
	\end{center}
	\pause
	\medskip
	\begin{center}
		L'appel est réalisé à chaque séance de TP/ TD $\to$ prévenez vos enseignant.e.s de vos absences !
	\end{center}
	\medskip
	Secrétariat L1 : Mme Ramatoulaye Barry, \url{ramatoulaye.barry@univ-eiffel.fr}
\end{frame}
\begin{frame}{Jours de congé}
	\begin{center}
		\includegraphics[width = 0.8\textwidth, keepaspectratio=True]{Images/Absences_religieuses.png}
	\end{center}
\end{frame}
...
\begin{frame}{Évaluation}
  ...
  \begin{block}{TPs}
    \begin{itemize}
    \item TP noté 1 la semaine du 02/10
    \item TP noté 2 la semaine du 14/12
    \item Une note de TP
      \onslide<3->{
      \item[$\Rightarrow$] TPs - \textbf{Coefficient 4}
      }
    \end{itemize}
  \end{block}
...
\begin{frame}{Bienvenue !}
    ...
    \begin{itemize}
 		\item Pour les débutants en programmation et/ou en Python
    ...
    \pause
    \vfill
    \begin{block}{À la fin de cours, vous saurez ...}
        \begin{itemize}
            \item Comprendre et écrire des programmes Python simple
            \item Écrire un petit jeu vidéo jouable sur une grille (Projet 1 !)
...
\begin{frame}{Liens utiles}
    ...
    \begin{itemize}
        ...
        \item {Page Elearning du cours} (avec notes de cours !) : \url{https://elearning.univ-eiffel.fr/course/view.php?id=16399}
        \item Thonny : \url{https://thonny.org/}
        \item Python Tutor : \url{https://pythontutor.com/}
    \end{itemize}
\end{frame}

\section{Une brève histoire de l'informatique}

\begin{frame}[t]{Une histoire de zéros et de uns\dots{}}
	\begin{center}
...
\begin{frame}{Une histoire de zéros et de uns\dots}
\begin{center}
\begin{tikzpicture}
  \node[pill, text width=5cm] (hn) at (0,2) {LANGAGES DE HAUT NIVEAU};
  \node[right=0.3cm of hn, draw=none] {\Large$\Rightarrow$};
  \node[font=\bfseries\Large, blue!60!black] at (0,0.8) {1954 : Fortran};
  \node[font=\bfseries\large, blue!60!black] at (0,-0.1) {(Mot « ordinateur » : 1955~!)};
\end{tikzpicture}

\onslide<2->{
\includegraphics[width=0.30\textwidth]{img/lettre_ordinateur.png}
}
\end{center}
\end{frame}

\begin{frame}{Une histoire de zéros et de uns\dots\ --- Généalogie}
\begin{center}
\begin{tikzpicture}[scale=0.72, every node/.style={scale=0.72},
  lang/.style={font=\small, align=center},
  >=latex
]
  \node[lang] (fortran) at (0,6)   {Fortran\\1954};
  \node[lang] (algol)   at (0,4.8) {Algol\\1958};
  \node[lang] (basic)   at (0,3.4) {Basic\\1964};
  \node[lang] (cobol)   at (2.4,4.8) {Cobol\\1959};
  \node[lang] (pascal)  at (2.4,3.4) {Pascal\\1970};
  \node[lang] (c)       at (-2.6,2) {C\\1972};
  \node[lang] (ada)     at (2.4,2)  {Ada\\1980};
  \node[lang] (python)  at (-0.4,0.6) {Python\\1991};
  \node[lang] (java)    at (2.4,0.6) {Java\\1995};
  \node[lang] (js)      at (-4.4,-0.8) {Javascript\\1995};
  \node[lang] (ruby)    at (-1.4,-0.8) {Ruby\\1995};
  \node[lang] (go)      at (-1.4,-2.2) {Go\\2009};
  \node[lang] (scala)   at (2.4,-0.8) {Scala\\2003};
  \node[lang] (kotlin)  at (2.4,-2.2) {Kotlin\\2011};
  \node[lang] (caml)    at (5.2,2)   {Caml\\1987};
...
% ===================================================================
% 2. De Python au binaire (pile de compilation)
% ===================================================================

\begin{frame}[t]{Une histoire de zéros et de uns\dots}
\begin{center}
\begin{tikzpicture}[scale=0.78, every node/.style={scale=0.78}]
  \node[pill] (l1) at (0,6.3) {LANGAGES DE HAUT NIVEAU};
  \onslide<2->{\node[pill] (l2) at (0,5.3) {ASSEMBLEUR};
    \draw[-latex] (l1) -- (l2);}
  \onslide<3->{\node[pill] (l3) at (0,4.3) {SYSTÈME D'EXPLOITATION};
    \draw[-latex] (l2) -- (l3);}
  \onslide<4->{\node[pill] (l4) at (0,3.1) {LANGAGE MACHINE};
    \draw[-latex] (l3) -- (l4);}
  \onslide<5->{\node[pill] (l5) at (0,2.1) {MICRO-CODE};
    \draw[-latex] (l4) -- (l5);}
  \onslide<6->{\node[pill] (l6) at (0,1.1) {PORTES LOGIQUES};
    \draw[-latex] (l5) -- (l6);}

  \onslide<7->{
    \draw[thick, orange] ($(l1.north west)+(-0.3,0.25)$) rectangle ($(l3.south east)+(0.3,-0.15)$);
    \node[pillsmall, left=1.4cm of l1] (log) {LOGICIEL};
    \draw[-latex, thick, orange] (log) -- ($(l1.west)+(-0.3,0)$);

    \draw[thick, orange] ($(l4.north west)+(-0.3,0.15)$) rectangle ($(l6.south east)+(0.3,-0.25)$);
    \node[pillsmall, left=1.4cm of l5] (mat) {MATÉRIEL};
    \draw[-latex, thick, orange] (mat) -- ($(l4.west)+(-0.3,0)$);
  }
  \onslide<8->{
    \node[pillsmall, right=1.6cm of l1] (py) {PYTHON};
    \draw[-latex, thick, orange] (py) -- (l1.east);
    \node[pillsmall, right=1.6cm of l5] (bin) {BINAIRE};
    \draw[-latex, thick, orange] (bin) -- (l4.east);
  }
  \onslide<9->{
    \draw[decorate, decoration={brace, amplitude=6pt}, thick]
      ($(l1.north east)+(1.0,0.2)$) -- ($(l6.south east)+(1.0,-0.2)$)
      node[midway, right=8pt, font=\bfseries] {Compilation};
  }
\end{tikzpicture}
\end{center}
\end{frame}
...
% ===================================================================
% 3. Algorithmique & Programmation 1
% ===================================================================

\begin{frame}{Exemples d'algorithmes ?}
\begin{itemize}
\item Au quotidien :
  \begin{itemize}
  \item Lacer ses chaussures
  \item Tricoter
  \item Conduire
  \item Faire des pancakes
  \end{itemize}
\item En mathématiques :
  \begin{itemize}
  \item Algorithme d'Euclide pour trouver le PGCD de deux entiers
  \item Tracer la médiatrice / bissectrice d'un angle (compas, etc\dots)
  \item Changement de base (binaire $\leftrightarrow$ décimal $\leftrightarrow$ hexadécimal)
  \end{itemize}
\item En informatique :
  \begin{itemize}
  \item Calculer le minimum d'un ensemble d'entiers
  \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Exemple : lacer ses chaussures}
Pour chaque chaussure :
\begin{itemize}
\item Croiser les lacets
\item Faire une boucle avec le lacet droit
\item Entourer la boucle avec le lacet gauche
\item Fermer le n\oe ud
\item Si lacets glissants : faire un double n\oe ud
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Exemple : calculer le minimum d'un ensemble d'entiers}
\begin{semiverbatim}
Entrée : E ensemble d'entiers
min_courant = e_0 élément de E
Pour chaque élément e de E :
    Si e < min_courant :
        min_courant = e
\textcolor{orange}{Renvoyer} min_courant
\end{semiverbatim}
\end{frame}

\section{De quoi parle-t-on ?}

\begin{frame}{Algorithmique \& Programmation 1}
\begin{itemize}
\item Algorithmique vs Programmation ?
\end{itemize}
\end{frame}
...
\begin{frame}{Algorithmique ? Programmation ? }
    ...
    \begin{block}{Algorithmique : Concevoir une solution}
      \vspace{0pt}

        Quelle est \textbf{l'idée} pour résoudre ce problème ?

        Quelle \textbf{méthode} utiliser ?
    \end{block}
    \pause
    \begin{center}
    ...
    \begin{block}{Programmation : Exécuter une solution}
      \vspace{0pt}

    On \og explique \fg{} à une machine comment mettre en place la solution.
    Étape par étape, dans un certain \textbf{langage de programmation}.
    \end{block}
...
\begin{frame}[fragile]{Mise en pratique}
\MVRightarrow{} Dessiner un carré.

cf \href{https://fr.vittascience.com/python/?mode=mixed&console=bottom&localId=loc65b6edf0196fa0}{Vittascience}.
...
\begin{center}
    ...
    \only<2>{ \includegraphics[width = 0.9\textwidth, keepaspectratio=True]{Images/logo_python.png}}
    \uncover<3->{
        Pour démarrer, on va devoir mettre la main à la pâte. \\
        \only<3>{ \includegraphics[width = 0.9\textwidth, keepaspectratio=True]{Images/main_a_la_pate.png}}
    }
    \uncover<4->{
         Quand on sera des grands chefs, on aura des commis de cuisine.
         \only<4>{ \includegraphics[width = 0.9\textwidth, keepaspectratio=True]{Images/commis_cuisine.png}}
         \only<5>{ \includegraphics[width = 0.5\textwidth, keepaspectratio=True]{Images/ChatGPT.png}}
    }
\end{center}
\end{frame}

\begin{frame}{Intermède : À propos de l'utilisation de l'IA en AP1}

  Voir les slides sur le sujet sur l'E-learning.

\end{frame}

\section{Python : qu'est-ce que c'est ?}
...
\begin{frame}[fragile]{Python \og calculatrice \fg{}}
	\begin{lstlisting}[style=python]
		2 + 3
	\end{lstlisting}
	\begin{onslide}<2->
...
\begin{frame}[t]{Qu'est-ce qu'une variable ?}
	...
	\begin{center}
		\begin{tikzpicture}
			\only<2>{\node (boite) {\includegraphics[width=3.6cm]{Images/Carton fermé.png}};}%
			\only<3>{\node (boite) {\includegraphics[width=3.6cm]{Images/Carton fermé_x.png}};}%
			\only<4>{%
				\node (boite) {\includegraphics[width=3.6cm]{Images/Carton ouvert_x.png}};
			...
			\only<8>{%
				\node (boitex) at (0,0) {\includegraphics[width=3.6cm]{Images/Carton fermé_x.png}};
				\node (boitey) at (5.5,0) {\includegraphics[width=3.6cm]{Images/Carton fermé_y.png}};
			}%
			% etape 9 : x s'ouvre, pointe vers 2 ; y reste ferme
			\only<9>{%
				\node (boitex) at (0,0) {\includegraphics[width=3.6cm]{Images/Carton ouvert_x.png}};
				\node (boitey) at (5.5,0) {\includegraphics[width=3.6cm]{Images/Carton fermé_y.png}};
			...
			\only<10>{%
				\node (boitex) at (0,0) {\includegraphics[width=3.6cm]{Images/Carton ouvert_x.png}};
				\node (boitey) at (5.5,0) {\includegraphics[width=3.6cm]{Images/Carton ouvert_y.png}};
				\node[font=\Huge\bfseries] (deux) at ($(boitex.east)+(0.9,0.9)$) {2};
				\draw[-latex, thick] (boitex.east) to[bend left=15] (deux.west);
			...
			\only<11>{\node (boite) at (0,0) {\includegraphics[width=3.6cm]{Images/Carton fermé_x.png}};}%
			% etape 12 : x ouvert pointe vers 2, "Salut" apparait ailleurs, non relie
			\only<12>{%
				\node (boite) at (0,0) {\includegraphics[width=3.6cm]{Images/Carton ouvert_x.png}};
...
\begin{frame}[fragile]{Booléens (\texttt{bool})}
	...
	\end{lstlisting}
	\begin{center}
		\lstinputlisting[style=pythonoutput]{code/correct_true_output.py}
	\end{center}
	\begin{lstlisting}[style=python]
		"AP1" == "Le meilleur cours de la L1"
	\end{lstlisting}
	\begin{center}
		\lstinputlisting[style=pythonoutput]{code/correct_False_output.py}
	\end{center}
...
\section{Annexes (pas vu en CM)}

\begin{frame}[t]{Modèle mémoire de Python}

	\only<1>{\lstinputlisting[style=python,linerange={1-1}]{code/memoire_trace.py}}%
	\only<2>{\lstinputlisting[style=python,linerange={1-2}]{code/memoire_trace.py}}%
	\only<3->{\lstinputlisting[style=python,linerange={1-3}]{code/memoire_trace.py}}%
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\only<1-2>{\draw[-latex, thick] (x.east) -- (v42.west);}
			% etape 2 : y pointe (a tort, pedagogiquement) directement vers x
			% \only<2>{\draw[-latex, thick] (y.north) -- (x.south);}
			% etape 2+ : correction, y pointe vers l'objet vise par x, pas vers x lui-meme
			\onslide<2->{\draw[-latex, thick] (y.east) -- (v42.west);}
			\onslide<3->{\draw[-latex, thick] (x.east) -- (v43.west);}
		\end{tikzpicture}
	\end{center}
...
% \begin{frame}[t]{Briques de base}
% \begin{center}
% \begin{tikzpicture}
%   \node[pill] (donnees) at (0,0) {DONNÉES};
%   \onslide<2->{\node[pill, right=0.3cm of donnees] (es) {ENTRÉES / SORTIES};}
%   \onslide<3->{\node[pill, right=0.3cm of es] (ops) {OPÉRATIONS};}
%   \onslide<4->{\node[pill, above=0.3cm of donnees, xshift=1.5cm] (sc) {STRUCTURES DE CONTRÔLE};}
%   \onslide<5->{\node[pill, above=0.3cm of ops] (fn) {FONCTIONS};}
% \end{tikzpicture}
% \end{center}
% \end{frame}

% \begin{frame}{Briques de base --- détail}
% \begin{center}
% \begin{tikzpicture}[scale=0.68, every node/.style={scale=0.68}]
%   \node[pill, text width=6.2cm, minimum height=2.4cm, align=center] (sc) at (0,1.6) {\phantom{x}};
%   \node[align=center, font=\bfseries\small, yshift=0.7cm] at (sc) {STRUCTURES DE CONTRÔLE};
%   \node[pillsmall, below=0.15cm of sc.center, xshift=-1.6cm, yshift=-0.55cm] (seq) {SÉQUENCES};
%   \node[pillsmall, right=0.2cm of seq] (cond) {CONDITIONS};
%   \node[pillsmall, below=0.2cm of seq, xshift=1.4cm, text width=3.6cm] (boucles) {BOUCLES\\(FOR ET WHILE)};

%   \node[pill, text width=6.2cm, minimum height=2.4cm, align=center] (donnees) at (0,-2.6) {\phantom{x}};
%   \node[align=center, font=\bfseries\small, yshift=0.7cm] at (donnees) {DONNÉES};
%   \node[pillsmall, below=0.15cm of donnees.center, xshift=-1.6cm, yshift=-0.55cm] (assig) {ASSIGNATION};
%   \node[pillsmall, right=0.2cm of assig] (vars) {VARIABLES};
%   \node[pillsmall, below=0.2cm of assig] (dict) {DICTIONNAIRES};
%   \node[pillsmall, right=0.2cm of dict] (listes) {LISTES};

%   \node[pill, right=1.3cm of sc] {ENTRÉES / SORTIES};
%   \node[pill, right=1.3cm of donnees] {OPÉRATIONS};
%   \node[pill, right=4.6cm of sc, yshift=1.5cm] {FONCTIONS};
% \end{tikzpicture}
% \end{center}
% \end{frame}
